...
public class TransactionNodeQuery implements Query<Integer> {
	...
	private static final String GET_ADDR_OUTPUT_TX = "SELECT tx_id FROM output WHERE address = ?",

	GET_NEXT_TX = "SELECT spent_by_tx" + " FROM output" + " WHERE spent_by_tx IS NOT NULL";
	...
	public String resultToString() {
		if (this.result < 1)
			return "No Path of Transactions found between [" + addrA + "] and [" + addrB + "]";
		else
			return "Shortest Path of Transactions  between [" + addrA + "] and [" + addrB + "] : " + this.result;
	}
