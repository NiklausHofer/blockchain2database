-- Find all transactions depending on a given transaction
DELIMITER //
CREATE PROCEDURE depend_on_tx( tx VARCHAR(255) )
  BEGIN
    DECLARE txid INT;
...
-- Recursive function to follow a transaction-graph
CREATE PROCEDURE rec_follow_graph( tx BIGINT, depth INT )
`rec_follow_graph`:
  BEGIN
